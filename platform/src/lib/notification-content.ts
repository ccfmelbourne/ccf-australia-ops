...
// Deliberately short: the attached voucher PDF (voucher-pdf.tsx) is the
// official document and carries every detail, including bank details,
// which must never appear in the email body itself. Request type +
// ministry are the one exception -- quick-scan triage info that lets
// Finance tell what a voucher's about without opening the PDF.
export function buildApprovedRequestEmail(detail: ApprovedRequestDetail): {
  ...
  // "No receipts were attached" reads as an actual notification; "including
  // 0 receipts" read like a debug message.
  const receiptClause =
    receiptCount === 0
